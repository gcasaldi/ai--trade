...
### Email position alerts (paper mode)

The realtime runner can email material target-weight changes as `APRI`, `AUMENTA`,
`RIDUCI`, `CHIUDI`, or `INVERTI`. The included `configs/live_stub.yaml` sends them to
`recipient@example.com` and keeps the broker on the local stub.

Create `.env` from `.env.example` and set a Gmail sender plus a Google App Password:

```dotenv
ALERT_SMTP_USER=your.sender@gmail.com
ALERT_SMTP_APP_PASSWORD=your_google_app_password
```

Then run the polling loop without `--execute`:

```bash
python scripts/run_realtime.py --config configs/live_stub.yaml --poll-seconds 300
```

Sent targets are persisted in `outputs/alert_state.json`, so unchanged decisions are
not emailed again after a restart. Delete that file only when you intentionally want
to reset alert history.

This is near-real-time scheduling over Yahoo daily bars, not a tick or intraday market
feed. Polling every five minutes checks for a newly available daily close; it does not
make this monthly-rebalance strategy suitable for intraday execution.

### Faster mobile alerts with Telegram

Telegram push notifications can be used instead of email or together with it. Create
a bot with `@BotFather`, send `/start` to the new bot, then put these values in `.env`:
...
To obtain the chat ID after sending `/start`, open
`https://api.telegram.org/bot<YOUR_TOKEN>/getUpdates` and read
`result[0].message.chat.id`. Keep the bot token private. The next material position
change will be delivered as a Telegram push notification.
